...
import { LIVING_EXPENSE_CATEGORY_LABELS, totals, type Pfs } from '../../lib/pfs'
...
export default function CashFlowSection({ pfs }: Props) {
  ...
  // Income — no categories in this model, so show each source individually.
  const incomeBuckets: Bucket[] = pfs.income
    .map((i, idx) => ({ key: i.id, label: i.label, amount: i.monthly, color: INCOME_PALETTE[idx % INCOME_PALETTE.length] }))
    .sort((a, b) => b.amount - a.amount)

  // Outflow: debt payments (mortgage P&I, secured, unsecured) + living spend by category.
    ...
    <section className="rounded-2xl border border-surface-200 bg-white p-6 shadow-card">
      ...
      {empty ? (
        ...
        <div className="mt-6 grid gap-7 lg:grid-cols-[1fr_auto] lg:items-center">
          ...
          <DiscretionaryDonut outflow={outflowBuckets} discretionary={discretionary} income={totalIncome} />
        </div>
      )}
    </section>
  )
  ...
  return (
    <div>
      ...
      <div
        ...
        role="img"
        aria-label={`${heading} ${formatUSD(total)} per month broken down`}
      >
        ...
        {buckets.map((b) => {
          ...
          return (
            <li key={b.key} className="flex items-center gap-2 text-xs text-surface-600">
              <span className="inline-block h-2.5 w-2.5 flex-shrink-0 rounded-sm" style={{ backgroundColor: b.color }} aria-hidden="true" />
