...
export default function BlogPost() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchPostBySlug(slug), fetchPublishedPosts()])
      .then(([p, all]) => {
        if (cancelled) return
        setPost(p)
        setOtherPosts(all.filter((o) => o.slug !== slug).slice(0, 3))
      })
      .catch(() => {
        // Failure here looks the same as a missing post — render NotFound
        // rather than a half-broken article shell.
      })
      .finally(() => {
  ...
  usePageMeta({
    title: post ? post.title : loading ? 'Loading…' : 'Post not found',
...
function BlogPostSkeleton() {
  return (
    <Container size="md" className="py-16">
      ...
      <div className="mt-8 h-12 w-3/4 animate-pulse rounded bg-surface-100" />
      <div className="mt-4 h-5 w-2/3 animate-pulse rounded bg-surface-100" />
      <div className="mt-10 aspect-[21/9] w-full animate-pulse rounded-2xl bg-surface-100" />
      <div className="mt-8 space-y-3">
        {[0, 1, 2, 3, 4].map((i) => (
          <div key={i} className="h-4 w-full animate-pulse rounded bg-surface-100" />
        ))}
      </div>
    </Container>
  )
